...
void graphe::brutForce()const
{
    std::bitset<5> nb_Bit;
    std::vector<std::bitset<5>>non_dominee;
    std::vector<std::bitset<5>>dominee;
    for (size_t i=0;i<pow(2,m_taille);++i)
    {
        nb_Bit=i;
        if(nb_Bit.count()==m_ordre-1)
        {
            std::cout<<nb_Bit<<std::endl;
            dominee.push_back(nb_Bit);
        }
...
int graphe::tailleS() const
{
     return m_sommets.size();
}


void graphe::insererArete (Aretes a)
{
    m_aretes.push_back(a);
}


void graphe::insererSommet (Sommet a)
{
...
void graphe::dessinerKu(BITMAP* page)
{
        int noir = makecol (0,0,0);
       // int rouge = makecol (200,0,0);
        for (size_t j=0; j<m_aretes.size(); j++)
        {
          ...
          //  float c = m_aretes[j].getpoids1();
          //  float d = m_aretes[j].getpoids2();
            line(page, ((m_sommets[a].getx())/2+450),((m_sommets[a].gety()+50)/2+250), ((m_sommets[b].getx())/2+450),(((m_sommets[b].gety()+50))/2+250), noir);
           // textprintf_ex(page,font,((m_sommets[a].getx()+m_sommets[b].getx())/2+2)/2+450, ((m_sommets[a].gety()+m_sommets[b].gety())/2+50+2)/2+250, rouge,-1,"%.0f/%.0f", c,d);
        }
        for (size_t i=0; i<m_sommets.size(); i++)
            {m_sommets[i].dessinerKu(page);}
}
